...
export default {
  ...
  watch: {
    ...
    // 播放时间的开始和结束
    curTime () {
      this.nowTime = this.formatSeconds(this.curTime)        //当前播放时间
      this.songTime = this.formatSeconds(this.duration)     //总时长
      // 移动进度条
  ...
  mounted () {
    this.progressLength = this.$refs.progress.getBoundingClientRect().width    //获取进度条的视窗位置，最大移动距离不可超过进度条宽度
  },
  methods: {
    // 切换音乐播放/暂停
    togglePlay () {
    ...
    //  解析播放时间
    formatSeconds (value) {
      let theTime = parseInt(value)         //换成秒
      let theTime1 = 0
      let theTime2 = 0
      if (theTime > 60) {
